...
public:
    ...
    // void test_speed()
    // {
    //     MatrixType x = {{0, 1}, {1, 0}};
    //     complex_type s2 = {1 / sqrt(2), 0};
    //     MatrixType h = {{s2, s2}, {s2, -s2}};
    //     Indexs id1 = {0};
    //     Indexs ct = {};
    //     apply_controlled_gate(x, id1, ct);
    //     for (unsigned i = 0; i < 26; i++)
    //     {
    //         Indexs id = {i};
    //         apply_controlled_gate(h, id, ct);
    //     }
    //     id1[0] = 25;
    //     apply_controlled_gate(x, id1, ct);
    // }
...
//                                    : N) schedule(static)
    ...
    void test(unsigned id0, MatrixType const &m, std::size_t ctrlmask, unsigned len)
    {
#pragma omp parallel
        kernel(vec_, id0, m, ctrlmask, len);
    }
    void print_info()
    {
        for (unsigned i = 0; i < ((len_ > 16) ? 16 : len_); i++)
        {
...
#pragma omp parallel
        ...
        {
            std::cout << "Gates with qubits " << ids.size() << std::endl;
